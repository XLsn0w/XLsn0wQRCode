...
@implementation XLsn0wQRcodeScaner
 ...
 */
- (void)addUI{
    self.maskView = [[MaskView alloc] initWithFrame:CGRectMake(0, 0, kScreen.width, kScreen.height)];
    ...
    if ([[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeLeft || [[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeRight){
        ...
        self.cancelButton.center = CGPointMake(kScreen.width - (self.view.center.x - kScreen.height * ZFScanRatio * 0.5) * 0.5, self.view.center.y);
...
- (void)capture{
    ...
    self.previewLayer.frame = CGRectMake(0, 0, kScreen.width, kScreen.height);
